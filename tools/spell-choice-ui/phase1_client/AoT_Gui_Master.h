...
public:
    ...
    // Phase 1: parse-only hook target for inbound chat lines.
    void OnInboundChatMessage(const std::string& line);
    const SpellChoice_UI& GetSpellChoiceState() const;
...
private:
    ...
    bool running = false;
    SpellChoice_UI spellChoiceState;
};
