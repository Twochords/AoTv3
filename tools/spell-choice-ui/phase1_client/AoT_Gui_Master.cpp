...
void HandleInboundChatLine(const std::string& line)
{
    gui_master.OnInboundChatMessage(line);
}
...
void AoT_Gui_Master::OnInboundChatMessage(const std::string& line) {
    if (!spellChoiceState.ParseOpenPayloadFromChat(line)) {
        return;
    }

    // Phase 1 deliberately stops at parser/state capture.
    printf("[AoT_Gui_Master] SC_OPEN captured for overlay staging.\n");
}

const SpellChoice_UI& AoT_Gui_Master::GetSpellChoiceState() const {
    return spellChoiceState;
}
